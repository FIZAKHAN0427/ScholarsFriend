        ...
        if url:
            try:
                ...
                if gbc:  # Check if the model loaded successfully
                    ...
                    return render_template("index.html", xx=-1, error="Model could not be loaded.")
            except Exception as e:
                print(f"Prediction error: {e}")
                return render_template("index.html", xx=-1, error="Error during prediction. Please check the URL and try again.")
        else:
            return render_template("index.html", xx=-1, error="URL not provided")
